...
from src.domain.models.game import NO_CURRENT_PLAYER, Game
...
from .conftest import BIG_BLIND_STANDARD, LARGE_CHIPS, MEDIUM_CHIPS, make_card, make_hand
...
class TestRaiseAction:
    ...
    def test_last_raise_increment_preserved_through_multiple_calls_after_raise(
        ...
        game_after_call_b = ActionApplier.apply_action(game_after_raise, player_b, call_action)
        assert game_after_call_b.betting_state.last_raise_increment == raise_increment

        game_after_call_c = ActionApplier.apply_action(game_after_call_b, player_c, call_action)
...
class TestGameStatePreservation:
    ...
    def test_preserves_table_positions_after_action(
        self,
        sample_player_factory: Callable[..., Player],
        minimal_game_factory: Callable[..., Game],
    ) -> None:
        ...
        assert updated_game.table_positions == game.table_positions
