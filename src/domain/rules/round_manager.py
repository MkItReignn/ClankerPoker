...
from src.domain.models.game import BettingState, Game, HandPhase, HandState
...
class RoundManager:
    ...
    def advance(game: Game) -> Game:
        ...
        if not players_in_hand:
            raise ValueError(
                "Cannot advance betting round: no players in hand"
            )
        ...
        updated_players: Players = game.players.transform_all(
            lambda p: p.reset_for_new_round()
        )

        next_phase: HandPhase | None = (
            game.hand_state.current_phase.next_phase()
        )
        if next_phase is None:
            raise ValueError(
                f"Cannot advance beyond {game.hand_state.current_phase}"
            )

        updated_community_cards: list[Card] = list(
            game.hand_state.community_cards
        )
        ...
        players_in_hand_for_next_phase: tuple[Player, ...] = (
            updated_players.in_hand()
        )
    ...
    def deal_community_cards(game: Game, deck: Deck) -> tuple[Game, Deck]:
        ...
        if current_phase == HandPhase.PRE_FLOP:
            raise ValueError(
                "Cannot deal community cards: preflop has no community cards"
            )
        if current_phase == HandPhase.SHOWDOWN:
            raise ValueError(
                "Cannot deal community cards: already at showdown"
            )

        next_phase = current_phase.next_phase()
        if next_phase is None:
            raise ValueError(
                f"Cannot deal community cards: no next phase after {current_phase}"
            )
        ...
        updated_community_cards = (
            list(game.hand_state.community_cards) + new_cards
        )
