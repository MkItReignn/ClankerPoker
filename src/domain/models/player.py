...
class Player:
    ...
    total_invested_this_hand: ChipAmount = field(
        default_factory=lambda: ChipAmount(0)
    )
    hands_played: int = 0
    ...
    stack_at_hand_start: ChipAmount | None = (
        None  # For elimination tiebreaker (SIMUL-004)
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.remaining_chips.value < 0:
            raise ValueError(
                f"Remaining chips cannot be negative: {self.remaining_chips.value}"
            )
        ...
        if self.hands_played < 0:
            raise ValueError(
                f"Hands played cannot be negative: {self.hands_played}"
            )
        if (
            self.elimination_hand_number is not None
            and self.elimination_hand_number < 1
        ):
            raise ValueError(
    ...
    def is_all_in(self) -> bool:
        ...
        return (
            self.total_invested_this_hand.value > 0
            and self.remaining_chips.value == 0
        )
    ...
    def reset_for_new_hand(self, hole_cards: Hand) -> Self:
    ...
    def reset_for_new_round(self) -> Self:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "hole_cards": (
                self.hole_cards.to_dict() if self.hole_cards else None
            ),
